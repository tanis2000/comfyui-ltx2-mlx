...
def _snap_frame_count_leq(target_frames: float) -> int:
    """Largest valid 8k+1 frame count that is <= target_frames.

    Unlike `_snap_frame_count` (which rounds to the *nearest* 8k+1, and can
    round up), this always rounds down. Used for match_audio_length so the
    derived video duration never exceeds the source audio's actual duration
    -- ltx_pipelines_mlx sizes its audio-token RoPE tables off the requested
    video duration regardless of how much real audio content exists, so
    asking for more video than there is audio crashes with a
    broadcast_shapes mismatch inside the audio attention RoPE step.
    """
    k = max(0, int((target_frames - 1) // 8))
    return 8 * k + 1
...
class LTX2MLXAudioToVideo(io.ComfyNode):
    ...
    def execute(
        ...
        if match_audio_length:
            frames = _snap_frame_count_leq(audio_duration * frame_rate)
        else:
